...
function checkstr(lstr,str){	//进行判断
	var arr=lstr.split(" ")		//把字符创分割成数组 并保存
...
//通过多种方式获取元素 jquery
	...
	if (typeof selector=="string"){
		...
		}else if (selector.charAt(0)=="#") {//获取id
...
function mousewheel(obj,upfun,downfun){
	if (obj.addEventListener){
		obj.addEventListener("mousewheel",scrollfun,false)//scrollfun是回调函数
	...
	function scrollfun(e){
		...
		if (ev.preventDefault) {
			ev.preventDefault()//阻止浏览器默认 滚动条变化的行为
		}else{
			ev.returnValue=false;//浏览器兼容的属性
		}
		if (dir==-3||dir==120) {//向上滚动 
			upfun.call(obj,ev)
		}else if(dir==3||dir==-120){//向下滚动
			downfun.call(obj,ev)
		}
	}
}
...
/*
  obj   要操作的对象
  overfun   鼠标 移入 需要处理的函数
  outfun    鼠标 移出 需要处理的函数
*/
//适用于:下拉菜单
function hover (obj,overfun,outfun) {
	  if(overfun){
	    obj.onmouseover=function  (e) {
			  if(checkHover(e,obj)){//判断
			     overfun.call(obj,[e]);
			  }
		}
	  }
